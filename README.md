...
경기 단위 통계만으로는 풀경기 체감과 관련된 세부 이벤트 구성을 확인하기 어렵기 때문에, Kaggle football-events 데이터를 추가로 활용하였다. 해당 데이터는 경기 단위 요약 통계가 아니라 경기 중 발생한 이벤트를 행 단위로 기록한 이벤트 데이터이다.
...
본 프로젝트에서는 해당 이벤트 데이터를 HDFS에 적재한 뒤 Spark를 이용하여 시즌별·리그별 이벤트 구성 지표를 집계하였다. 이 데이터는 2008/09-2024/25 장기 변화 분석을 직접 대체하기보다는, 풀경기 체감과 관련될 수 있는 이벤트 단위 분석 가능성을 보완하는 역할로 사용하였다.

### 3.4 Kaggle European Soccer Database

Kaggle European Soccer Database는 SQLite 형태의 축구 데이터로, 본 프로젝트에서는 초기 데이터 수집, SQLite-to-CSV 변환, HDFS 적재 과정 검증에 사용하였다. 다만 해당 데이터는 2015/16 시즌까지만 포함하므로, 2016/17 이후 현대 축구 변화 분석의 핵심 데이터로는 사용하지 않았다. 최종 분석은 football-data.co.uk, Understat, football-events 데이터를 중심으로 수행하였다.

### 3.5 데이터 규모 요약

본 프로젝트에서는 장기 경기 통계, xG 기반 팀-경기 데이터, 이벤트 단위 데이터를 함께 사용하였다. 대용량 원본 데이터는 GitHub에 직접 업로드하지 않고, 실행 시 로컬과 HDFS에 저장되도록 구성하였다.

| 데이터                                  | 사용 목적                         |                             규모 |
| ------------------------------------ | ----------------------------- | -----------------------------: |
| football-data.co.uk Match Statistics | 2008/09-2024/25 장기 경기 통계 분석   |              85개 CSV, 30,790경기 |
| Kaggle Understat Data                | xG, PPDA, deep 기반 공격 기회 품질 분석 |                         약 27MB |
| Kaggle football-events               | 이벤트 단위 Spark 분석               | events.csv 약 173MB, 전체 약 196MB |
| Kaggle European Soccer Database      | SQLite 변환 및 HDFS 적재 과정 검증     |                        약 300MB |
...
| 분산 처리   | Apache Spark, Apache Hive | Spark는 주요 집계 분석과 이벤트 데이터 처리에 사용하고, Hive는 HDFS 기반 SQL 보조 집계 및 Spark 결과 검증에 사용 |
| 추가 분석   | Python                    | 변화 지점 탐색, xG 분석, 빅게임 분석                                                      |
| 시각화     | Python, Matplotlib        | 분석 결과 그래프 생성                                                                 |
| 버전 관리   | Git, GitHub               | 코드 및 결과 관리                                                                   |

본 프로젝트는 HDFS를 대용량 데이터 저장 계층으로 사용하고, Apache Spark를 핵심 분산 처리 엔진으로 활용하였다. 또한 Hive를 이용해 HDFS에 저장된 표준화 경기 데이터를 외부 테이블로 등록하고, SQL 기반 시대 구간별 요약 통계를 산출하여 Spark 분석 결과를 보조 검증하였다. 이를 통해 강의에서 다룬 Hadoop 생태계의 저장 계층과 처리/SQL 계층을 함께 활용하는 데이터 파이프라인을 구성하였다.
...
5. HiveQL 보조 검증 분석
   - HDFS의 표준화 경기 데이터를 Hive 외부 테이블로 등록
   - 과거/현대 구간의 평균 득점, 평균 슈팅, 유효슈팅, 슈팅 전환율을 SQL 기반으로 재집계
   - Spark 분석 결과와 동일한 방향의 결과가 나오는지 검증
...
8. 시각화
   ...
   - xG 기반 공격 기회 품질 변화
   - 빅게임의 슈팅·득점 변화
   - football-events 기반 시즌별 이벤트 구성 변화
```
...
HiveQL 보조 검증 분석은 HiveServer2가 실행된 상태에서 다음 명령으로 수행할 수 있다.

```bash
beeline -u jdbc:hive2://localhost:10000/default -n hive -f src/pipeline/analyze_match_dynamics_hive.hql
```

대용량 원본 데이터는 GitHub에 업로드하지 않고, 실행 시 로컬 및 HDFS에 저장되도록 구성하였다. GitHub에는 재현성과 구조 확인을 위한 샘플 데이터만 `data/sample`에 포함하였다.
...
│   ├── README.md
│   └── sample
│       ├── football_data_matches_sample.csv
│       ├── football_events_games_sample.csv
│       ├── football_events_sample.csv
│       └── understat_game_stats_sample.csv
├── scripts
│   └── run_pipeline.sh
├── src
│   ├── ingest
...
│   │   └── analyze_match_dynamics_hive.hql
│   └── analyze
...
│       └── event_season_metrics_trend.png
```
...
| --------- | -----: | ------: | ------: | ------: | ----------: | -----: | -----: |
| 2008-2012 | 2.6565 | 25.8350 | 10.0215 | 0.10465 |      0.5030 | 0.4711 | 0.2685 |
| 2012-2016 | 2.7008 | 25.1990 |  9.0438 | 0.10885 |      0.4960 | 0.4539 | 0.2932 |
| 2016-2020 | 2.7713 | 24.4245 |  8.9340 | 0.11563 |      0.4683 | 0.4572 | 0.2985 |
| 2020-2025 | 2.8182 | 24.7570 |  8.8232 | 0.11632 |      0.4694 | 0.4270 | 0.3191 |
...
football-events 데이터는 약 173MB 규모의 `events.csv`와 경기 메타데이터인 `ginf.csv`로 구성되며, 총 941,009개의 이벤트와 10,112경기를 포함한다. 본 프로젝트에서는 이 데이터를 HDFS에 적재한 뒤 Spark를 사용하여 시즌별·리그별 이벤트 구성 지표를 집계하였다.
...
시즌별 이벤트 분석 결과, 2012-2017 구간에서 경기당 이벤트 수, 슈팅 이벤트 수, 키패스, 파울, 카드 등 경기 내 이벤트 구성 지표를 확인할 수 있었다. 이 분석은 2008/09-2024/25 장기 변화 분석을 직접 대체하기 위한 것이 아니라, 축구의 재미와 경기 체감에 영향을 줄 수 있는 이벤트 단위 요소를 추가로 확인하기 위한 보조 분석이다.

### 8.8 HiveQL 기반 보조 검증 분석

Spark 분석 결과가 HDFS 기반 SQL 분석에서도 동일한 방향으로 나타나는지 확인하기 위해 HiveQL 보조 검증 분석을 수행하였다. HDFS에 저장된 표준화 경기 데이터를 Hive 외부 테이블로 등록한 뒤, 과거 구간과 현대 구간의 평균 득점, 평균 슈팅, 평균 유효슈팅, 슈팅 전환율을 집계하였다.

| 구간                |   경기 수 |  평균 득점 |   평균 슈팅 | 평균 유효슈팅 |  슈팅 전환율 |
| ----------------- | -----: | -----: | ------: | ------: | ------: |
| historical        | 14,607 | 2.6786 | 25.5170 |  9.5325 | 0.10675 |
| modern_transition | 16,183 | 2.7966 | 24.6075 |  8.8699 | 0.11600 |

HiveQL 결과에서도 과거 구간 대비 현대 구간에서 평균 슈팅과 유효슈팅은 감소했지만, 평균 득점과 슈팅 전환율은 증가하는 흐름이 확인되었다. 따라서 Spark로 도출한 핵심 결론이 Hive 기반 SQL 집계에서도 재현됨을 확인하였다.
...
### 9.13 football-events 기반 시즌별 이벤트 구성 변화

![Event season metrics trend](results/figures/event_season_metrics_trend.png)

football-events 데이터의 시즌별 경기당 이벤트 수, 슈팅 이벤트 수, 키패스 수, 파울 수를 비교하였다. 이 분석은 장기 경기 통계 분석을 직접 대체하기보다는, 풀경기 체감과 관련될 수 있는 이벤트 단위 구성 변화를 보조적으로 확인하기 위한 분석이다.
...
추가적으로 football-events 이벤트 데이터를 활용하여 경기당 이벤트 수, 슈팅 이벤트, 키패스, 빠른 공격, 파울 및 카드 등 경기 체감과 관련될 수 있는 이벤트 구성 지표를 산출하였다. 다만 해당 데이터는 2012-2017 구간에 한정되므로, 장기적인 현대 축구 변화 결론을 직접 대체하기보다는 이벤트 단위 분석 가능성을 보완하는 역할로 해석하였다.

또한 HiveQL을 활용한 HDFS 기반 SQL 보조 집계에서도 Spark 분석과 동일하게 슈팅 수 감소와 득점 효율 증가 흐름이 확인되어, 핵심 결과의 일관성을 추가로 검증하였다.

결론적으로 현대 축구는 “공격이 줄어든 축구”라기보다, “공격 기회를 더 선별적으로 사용하고 더 효율적으로 득점하는 축구”로 변화한 것으로 해석할 수 있다. 다만 본 프로젝트는 재미 자체를 직접 측정한 것이 아니라, 득점, 슈팅, xG, 빅게임, 이벤트 구성 지표를 통해 경기 양상 변화를 간접적으로 분석한 것이다. 따라서 현대 축구가 재미없게 느껴지는 원인을 더 정밀하게 설명하기 위해서는 백패스 비율, 전진패스 비율, 공이 중앙선 뒤에 머무르는 시간, 빌드업 시간, 드리블 시도, 경합 수와 같은 세부 이벤트·좌표 데이터가 추가로 필요하다.

## 11. 한계 및 확장 방향

본 프로젝트는 경기 단위 및 팀-경기 단위 통계를 중심으로 분석하였다. 따라서 축구의 “재미”를 직접적으로 측정했다고 보기는 어렵다. 특히 백패스 비율, 전진패스 비율, 공이 중앙선 뒤에 머무르는 시간, 빌드업 시간, 드리블 시도 수, 경합 수와 같은 지표는 풀경기 체감과 밀접하지만, 본 프로젝트에서 사용한 공개 데이터만으로는 정밀하게 계산하기 어렵다.

또한 football-events 데이터는 이벤트 단위 분석에 활용되었지만, 공개 데이터의 기간이 2012-2017에 한정되어 있어 2020년대 후반까지 이어지는 장기 변화를 직접 설명하기에는 한계가 있다. 패스 방향, 백패스 비율, 빌드업 시간 등을 정밀하게 계산하기 위해서는 패스 시작·종료 좌표와 이벤트 시퀀스가 포함된 더 세밀한 이벤트 데이터가 필요하다.
...
* ChatGPT: 디버깅, 코드 구조 점검, README 문서 정리, Matplotlib 시각화 아이디어 보조
